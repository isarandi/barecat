...
def test_barecat_creation(temp_jpeg_dir):
    ...
    with barecat.Barecat(output_file) as reader:
        ...
        assert (
            'dir1/subdir1/test1.jpg' in file_list
        ), 'Expected dir1/subdir1/test1.jpg in the archive'
        assert (
            'dir1/subdir2/test2.jpg' in file_list
        ), 'Expected dir1/subdir2/test2.jpg in the archive'
        assert 'dir2/test3.jpg' in file_list, 'Expected dir2/test3.jpg in the archive'
        assert (
            reader['dir1/subdir1/test1.jpg'] == b'dummy data1'
        ), "Expected dir1/subdir1/test1.jpg to contain 'dummy data1'"
        assert (
            reader['dir1/subdir2/test2.jpg'] == b'dummy data2'
        ), "Expected dir1/subdir2/test2.jpg to contain 'dummy data2'"
        assert (
            reader['dir2/test3.jpg'] == b'dummy data3'
        ), "Expected dir2/test3.jpg to contain 'dummy data3'"
...
def test_defrag(barecat_archive):
    ...
    defrag_cmd = ['barecat-defrag', str(barecat_archive)]
...
def test_defrag_smart_multishard(tmp_path):
    ...
    with barecat.Barecat(
        archive_path, readonly=False, overwrite=True, shard_size_limit=shard_size_limit
    ) as bc:
    ...
    with barecat.Barecat(archive_path) as bc:
        ...
        assert bc.total_logical_size == bc.total_physical_size_seek, 'Archive not fully compacted'
...
def test_reshard(barecat_archive):
    ...
    with barecat.Barecat(barecat_archive) as reader:
        ...
        _ = reader.sharder.num_shards

    # Reshard to larger limit (should consolidate shards)
    reshard_cmd = ['barecat', 'reshard', str(barecat_archive), '-s', '1M']
    ...
    reshard_cmd = ['barecat', 'reshard', str(barecat_archive), '-s', '15']
    result = subprocess.run(reshard_cmd, capture_output=True, text=True)
    assert result.returncode == 0, f'Command failed: {result.stderr}'
...
def test_index_to_csv(barecat_archive):
    ...
    assert (
        '"path","shard","offset","size","crc32c"' in result.stdout
    ), 'CSV output missing expected header'
    assert result.returncode == 0, f'Command failed: {result.stderr}'
