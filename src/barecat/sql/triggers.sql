-- Triggers for maintaining directory statistics
--
-- We propagate changes up the tree with triggers, as this is cumbersome to do in
-- the Python code. There is no propagation downwards (for example when moving a dir,
-- we do not update all the children with triggers). This is because the Python code
-- can do this quite easily. Furthermore, if we did it with triggers, the chain would
-- start upward again with a circular mess. So we only propagate upwards the tree.
--
...
-- this can be simply done in the Python code. If the app doesn't supply mtime,
-- presumably it doesn't care about it, so the overhead of triggering it makes no sense.
...
---- Cross-table constraints: prevent path from being both file and directory
CREATE TRIGGER prevent_file_as_dir
    BEFORE INSERT ON files
    WHEN (SELECT value_int FROM config WHERE key = 'use_triggers') = 1
BEGIN
    SELECT RAISE(ABORT, 'Path already exists as directory')
    WHERE EXISTS (SELECT 1 FROM dirs WHERE path = NEW.path);
END;

CREATE TRIGGER prevent_dir_as_file
    BEFORE INSERT ON dirs
    WHEN (SELECT value_int FROM config WHERE key = 'use_triggers') = 1
BEGIN
    SELECT RAISE(ABORT, 'Path already exists as file')
    WHERE EXISTS (SELECT 1 FROM files WHERE path = NEW.path);
END;
