...
def main():
    ...
    p.add_argument('-T', '--files-from', type=str, metavar='FILE',
    ...
    p.add_argument('-C', '--directory', type=str, default=None, metavar='DIR',
    ...
    p.add_argument('-s', '--shard-size-limit', type=str, default=None, metavar='SIZE',
                   help='Shard size limit (e.g., 1G, 500M)')
    p.add_argument('--physical-order', action='store_true',
                   help='Add files in physical disk order (for HDDs)')
    ...
    p.add_argument('-c', '--create', action='store_true',
                   help='Create archive if it does not exist')
    p.add_argument('-s', '--shard-size-limit', type=str, default=None, metavar='SIZE',
    ...
    p.add_argument('paths', type=str, nargs='*',
                   help='Paths to extract (default: all)')
    p.add_argument('-C', '--directory', type=str, default='.',
                   help='Extract to directory (default: current dir)')
    p.add_argument('--pattern', type=str, default=None, metavar='GLOB',
                   help='Only include files matching glob pattern (incompatible with -i/-x)')
    p.add_argument('--exclude', '-x', action='append', default=[], metavar='PATTERN',
                   help='Exclude files matching glob pattern (repeatable)')
    p.add_argument('--include', '-i', action='append', default=[], metavar='PATTERN',
    ...
    p.add_argument('archive', type=str, help='Barecat archive path')
    p.add_argument('paths', type=str, nargs='*',
                   help='Paths or glob patterns to list (default: root)')
    p.add_argument('-l', '--long', action='store_true',
                   help='Long listing with sizes')
    ...
    p.add_argument('path', type=str, nargs='?', default='',
                   help='Starting path (default: root)')
    p.add_argument('-name', type=str, metavar='PATTERN',
                   help='Match basename against glob pattern')
    p.add_argument('-path', '-wholename', type=str, metavar='PATTERN', dest='pathpattern',
                   help='Match full path against glob pattern')
    p.add_argument('-type', type=str, choices=['f', 'd'], dest='ftype',
                   help='File type: f=file, d=directory')
    p.add_argument('-size', type=str, metavar='[+-]N[kMG]',
                   help='Size filter: +N (larger), -N (smaller), N (exact)')
    p.add_argument('-maxdepth', type=int, metavar='N',
                   help='Maximum depth to descend')
    p.add_argument('-print0', action='store_true',
                   help='Print paths separated by null character')
    ...
    p.add_argument('path', type=str, nargs='?', default='',
                   help='Path within archive (default: root)')
    p.add_argument('-L', '--level', type=int, default=None, metavar='N',
    ...
    p.add_argument('--quick', action='store_true',
                   help='Quick check (index only, no CRC)')
    ...
    p.add_argument('--max-seconds', type=float, default=300, metavar='SEC',
    ...
    p.add_argument('archive', type=str, help='Path to the archive')
    p.add_argument('-s', '--shard-size-limit', type=str, required=True, metavar='SIZE',
    ...
    # merge
    p = subparsers.add_parser('merge', help='Merge multiple archives (barecat, tar, zip) into one barecat')
    p.add_argument('archives', metavar='ARCHIVE', type=str, nargs='+',
                   help='Archives to merge')
    p.add_argument('-o', '--output', required=True, help='Output archive')
    p.add_argument('-s', '--shard-size-limit', type=str, default=None, metavar='SIZE',
    ...
    p.add_argument('--as-subdirs', action='store_true',
                   help='Put each archive in a subdir named after its basename (minus extension)')
    p.add_argument('--prefix', type=str, default=None, metavar='PATH',
    ...
    p = subparsers.add_parser('convert', help='Convert between barecat and tar/zip (auto-detects direction)')
    p.add_argument('input', type=str, help='Input file or format (tar.gz, tar.bz2, etc. with --stdin)')
    p.add_argument('output', type=str, help='Output file or format (tar.gz, tar.bz2, etc. with --stdout)')
    p.add_argument('-s', '--shard-size-limit', type=str, default=None, metavar='SIZE',
    ...
    p.add_argument('--root-dir', type=str, metavar='NAME',
    ...
    p.add_argument('--preserve-backup', action=misc.BoolAction, default=True,
    ...
    p.add_argument('-o', '--options', type=str, metavar='OPTIONS',
    ...
    p.add_argument('-c', '--checksum', action='store_true', help='Compare by checksum not size/mtime')
    ...
    if args.command in ('create', 'c'):
        ...
        if barecat.util.misc.exists(args.output) and not args.force:
            print(f"Error: Output '{args.output}' already exists. Use -f to overwrite.",
                  file=sys.stderr)
            sys.exit(1)
        ...
        if output_exists and not args.force and not args.append:
            print(f"Error: Output '{args.output}' already exists. "
                  "Use -f to overwrite or -a to append.", file=sys.stderr)
            sys.exit(1)
        ...
        if args.stdin:
            if args.input not in tar_formats:
                print(f"Error: With --stdin, input must be a tar format: {', '.join(tar_formats)}",
                      file=sys.stderr)
                sys.exit(1)
        ...
        if args.stdout:
            if args.output not in tar_formats:
                print(f"Error: With --stdout, output must be a tar format: {', '.join(tar_formats)}",
                      file=sys.stderr)
                sys.exit(1)
        ...
        if input_is_archive and output_is_archive:
            print("Error: Both input and output are tar/zip. One must be a barecat path.",
                  file=sys.stderr)
            sys.exit(1)
        if not input_is_archive and not output_is_archive:
            print("Error: Neither input nor output is tar/zip. "
        ...
        if input_is_archive:
            ...
            if args.wrap:
                ...
                if args.input.lower().endswith(compressed_exts):
                    print("Error: --wrap only works with uncompressed .tar or .zip files",
                          file=sys.stderr)
                    sys.exit(1)
                if args.stdin:
...
def _handle_add(args, overwrite=False, exist_ok=True):
            ...
            for root in args.paths:
                if osp.isfile(root):
                    ...
                    for dirpath, subdirnames, filenames in os.walk(root):
                        ...
                        subdirnames[:] = [d for d in subdirnames
                                          if should_include(osp.join(dirpath, d))]
    ...
    if not args.paths and not args.files_from:
        print('Error: No input specified. Provide paths or -T FILE.',
              file=sys.stderr)
        sys.exit(1)
...
def _handle_mount(args):
    ...
    try:
        ...
        print("Error: Mount support not installed. Install with: pip install barecat[mount]",
              file=sys.stderr)
        sys.exit(1)
    ...
    if args.options:
        for opt in args.options.split(','):
            ...
            if opt == 'ro':
                ...
                print("Valid options: ro, rw, fg/foreground, mmap, defrag, overwrite, "
                      "append_only, shard_size_limit=SIZE", file=sys.stderr)
                sys.exit(1)
